...
def run_news_agent():
    # 1. 뉴스 수집
    # 팁: q=AI+technology 부분을 수정하여 관심 분야 뉴스를 조절하세요.
    ...
    # 2. 모델 설정
    # 모델명은 실행 환경에 따라 'gemini-1.5-flash' 등으로 변경이 필요할 수 있습니다.
    model_name = 'gemini-1.5-flash' 
    model = genai.GenerativeModel(model_name)

    # 3. 프롬프트 설정 (수정하신 내용 반영)
    prompt = f"""
    ...
    try:
        ...
        # 날짜 데이터 생성
        now = datetime.now()
        today_file = now.strftime("%Y-%m-%d")    # 파일명용
        today_title = now.strftime("%Y/%m/%d")   # 제목용
